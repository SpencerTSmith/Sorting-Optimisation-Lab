...
#define USE_NANOPRECISION 0 // NOTE: If this breaks the build then set it to 0
...
#define TIMER_GET_CLOCK(_counter_)                                             \
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &_counter_);

#define TIMER_GET_DIFF(_start_, _stop_, _diff_)                                \
	{                                                                          \
		_diff_ = (_stop_.tv_sec - _start_.tv_sec) * 1000000000 +               \
				 _stop_.tv_nsec - _start_.tv_nsec;                             \
	}
...
#define TIMER_GET_DIFF(_start_, _stop_, _diff_)                                \
	_diff_ = ((_stop_.tv_sec - _start_.tv_sec) * 1e6 + _stop_.tv_usec -        \
			  _start_.tv_usec) *                                               \
			 1000; // <-- TODO: this 1000 should actually be derived like it is
				   // in memory mountatin
