...
extern void COMPUTE_NAME_REF(int m0, float *input_x, float *output_y);

extern void COMPUTE_NAME_TST(int m0, float *input_x, float *output_y);
    ...
    for (int p = min_size; p < max_size; p += step_size) {
      ...
      Run the reference
        */
        ...
        COMPUTE_NAME_REF(m0, input_ref, output_ref);

        // run the test
        // Perform the computation
        COMPUTE_NAME_TST(m0, input_tst, output_tst);

        // compute the error
        float res =
            max_pair_wise_diff(output_sz, 1, 1, 1, output_ref, output_tst);

        fprintf(result_file, "%i, ", m0);
